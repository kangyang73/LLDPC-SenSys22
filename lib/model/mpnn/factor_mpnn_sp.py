...
class FactorNN(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        self.node_mapping_module = iid_mapping(self.node_feature_dim, self.map_dim)
        ...
        final_dim = nclass if nclass > 2 else 1   # 1
    ...
    def forward(self, node_feature: torch.Tensor,
        ...
        nhop_feature = [m(f) for f, m in zip(hop_features, self.factor_mapping_modules)]

        all_inter_features = []

        len_v2f_modules = len(self.v2f_modules)
        for idx in range(len_v2f_modules):
            nin = self.dim_mapping_list[idx]
            nout = self.dim_mapping_list[idx + 1]
            nfeature = self.v2v_modules[idx](nnode_feature)
            nffeature = [m(f) for f, m in zip(nhop_feature, self.f2f_modules[idx])]
            
            len_f2v_modules = len(self.f2v_modules[idx])
            for jidx in range(len_f2v_modules):
                nv = self.mpnn_forward(
                    self.f2v_modules[idx][jidx], nhop_feature[jidx], nn_idx_f2v[jidx].long(), etype_f2v[jidx])
                nfeature = nfeature + nv

                nf = self.v2f_modules[idx][jidx](nnode_feature, nn_idx_v2f[jidx].long(), etype_v2f[jidx])
                nffeature[jidx] = nffeature[jidx] + nf

            if nin == nout:
                nnode_feature = nnode_feature + nfeature
                nhop_feature = [m1 + m2 for m1, m2 in zip(nffeature, nhop_feature)]
            else:
            ...
            if idx in self.skip_link.keys():
                ...
                nhop_feature = [m1 + m2 for m1, m2 in zip(offeature, nhop_feature)]
